...
const mutation = {
  async createArtist (root: {}, { name }: Artist, context: Context) {
    return context.db.mutation.createArtist({ data: { name } })
  },
...
const subscription = {
  createArtist: {
    async subscribe (root: {}, args: Artist, context: Context, info) {
      return context.db.subscription.artist({ where: { mutation_in: ["CREATED"] } }, info)
    }
  },

  updateArtist: {
    async subscribe (root: {}, args: Artist, context: Context, info) {
      return context.db.subscription.artist({ where: { mutation_in: ["UPDATED"] } }, info)
    }
  },

  deleteArtist: {
    async subscribe (root: {}, args: Artist, context: Context, info) {
      return context.db.subscription.artist({ where: { mutation_in: ["DELETED"] } }, info)
    }
  }
}
